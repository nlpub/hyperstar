...
for hyponym, _ in subsumptions_train:
...
for hyponym, _ in subsumptions_test:
    word_synonyms = synonyms[hyponym] if len(synonyms[hyponym]) > 0 else [hyponym]
...
np.savez_compressed('train-vectors.npz', X_all_train=X_all_train,
                                         Y_all_train=Y_all_train,
                                         Z_index_train=Z_index_train,
                                         Z_all_train=Z_all_train)

np.savez_compressed('test-vectors.npz',  X_all_test=X_all_test,
