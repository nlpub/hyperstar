...
class Baseline:
    ...
    def __init__(self, x_size, y_size):
        ...
        self.X = tf.placeholder(tf.float32, shape=[None, self.x_size + 1], name='X')
        self.Y = tf.placeholder(tf.float32, shape=[None, self.y_size], name='Y')
        self.Z = tf.placeholder(tf.float32, shape=[None, self.x_size + 1], name='Z')
        self.W = tf.Variable(tf.random_normal((self.x_size + 1, self.y_size), stddev=0.1), name='W')
